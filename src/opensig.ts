...
import { assertIBlockchainProvider, IBlockchainProvider } from './providers/IBlockchainProvider';
 ...
 */
let logTrace: Function = function() {};
setLogTrace(false);

export function setLogTrace(traceOn: boolean) {
...
export class OpenSig {
  ...
  constructor(public provider: IBlockchainProvider) {
...
export class Document {
   ...
   */
  async sign(data?: SignatureData): Promise<SignatureReceipt> {
    if (this.signingInProgress) throw new Error("Signing already in progress");
    if (!this.hashIterator) throw new Error("Document must be verified before signing");
    ...
    return this.hashIterator.next()
      ...
      .catch(error => {
        this.hashIterator?.reset(this.hashIterator.currentIndex()-1);
        throw error;
      })
      .finally(() => {
 ...
 */ 
async function _publishSignature(provider: IBlockchainProvider, signature: Uint8Array, encryptionKey: EncryptionKey, data?: SignatureData): Promise<SignatureReceipt> {
...
async function _discoverSignatures(provider: IBlockchainProvider, documentHash: Uint8Array, encryptionKey: EncryptionKey) {
  ...
  async function _discoverNext(n: number) {
    ...
    if (events.length === MAX_SIGS_PER_DISCOVERY_ITERATION) return _discoverNext(MAX_SIGS_PER_DISCOVERY_ITERATION);
...
export class HashIterator {
  ...
  async next(n=1): Promise<Uint8Array[]> {
    if (!this.chainSpecificHash) this.chainSpecificHash = await hash(concat([Uint8Array.from(''+this.chainId), this.documentHash]));
  ...
  current(): Uint8Array | undefined { return this.hashPtr >= 0 ? this.hashes[this.hashPtr] : undefined }

  currentIndex(): number { return this.hashPtr }

  indexAt(i: number): Uint8Array | undefined { return i < this.hashes.length ? this.hashes[i] : undefined }
  ...
  size(): number { return this.hashPtr+1 }
