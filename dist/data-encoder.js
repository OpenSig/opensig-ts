...
//
export async function encodeData(encryptionKey, data) {
    ...
    switch (data.type) {
        case 'string':
            ...
            break;
        case 'binary':
            if (typeof data.content !== 'string' || !isHexString(data.content))
                throw new Error("invalid data content");
...
export async function decodeData(encryptionKey, encData) {
    ...
    if (version !== SIG_DATA_VERSION)
        return { type: "invalid", content: "unsupported data version: " + version };
